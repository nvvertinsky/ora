...
Индекс в предполагаемом плане может отличаться от индекса в реальном плане в большинстве случаев из-за того что он строит план изходя из первых параметров. Показать параметры на основании которых построил план:
```
select *
  from table(dbms_xplan.display('sql_id', format=>'+peeked binds'))
```
